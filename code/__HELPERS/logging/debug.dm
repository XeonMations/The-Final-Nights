...
/proc/log_asset(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_ASSET, text, data)

/// Logging for config errors
/// Rarely gets called; just here in case the config breaks.
/proc/log_config(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_CONFIG, text, data)
	SEND_TEXT(world.log, text)

/proc/log_filter_raw(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_FILTER, text, data)

/// Logging for job slot changes
/proc/log_job_debug(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_JOB, text, data)

/// Logging for lua scripting
/proc/log_lua(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_LUA, text, data)
...
#endif
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_MAPPING, text)
	if(skip_world_log)
		return
	SEND_TEXT(world.log, text)
...
/proc/log_qdel(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_QDEL, text, data)
...
/proc/log_signal(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_SIGNAL, text, data)

/// Logging for DB errors
/proc/log_sql(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_DEBUG_SQL, text, data)

/// Logging for world/Topic
/proc/log_topic(text, list/data)
	GLOB.logger.Log(LOG_CATEGORY_GAME_TOPIC, text, data)
