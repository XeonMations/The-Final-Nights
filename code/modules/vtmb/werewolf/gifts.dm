		...
		for(var/mob/living/carbon/target in mobs_in_range)
			...
			if (iskindred(target))
				...
				if ((vampire.morality_path?.score < 7) || vampire.client?.prefs?.is_enlightened)
					is_wyrm = 1
				if ((vampire.clan?.name == "Baali") || ( (vampire.client?.prefs?.is_enlightened && (vampire.morality_path?.score > 7)) || (!vampire.client?.prefs?.is_enlightened && (vampire.morality_path?.score < 4)) ))
					is_wyrm = 1
