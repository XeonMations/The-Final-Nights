	...
	if(allowed_to_proceed)
		...
		if(do_after(owner, 3 SECONDS))
			...
			if(CheckZoneMasquerade(owner))
				...
				var/mob/living/carbon/human/werewolf/W
				if(ishuman(owner))
					H = owner
				else
					W = owner
...
/datum/action/gift/guise_of_the_hound/Trigger()
	...
	if(allowed_to_proceed)
		...
		if(istype(owner, /mob/living/carbon/human/werewolf/lupus))
			var/mob/living/carbon/human/werewolf/lupus/lopor = owner
...
/datum/action/gift/guise_of_the_hound/proc/transform_lupus(mob/living/carbon/human/werewolf/lupus/H)
...
/datum/action/gift/banish_totem/Trigger()
	...
	if(allowed_to_proceed)
		...
		for(var/mob/living/carbon/human/werewolf/wtarget in orange(7,owner))
			targets += wtarget
