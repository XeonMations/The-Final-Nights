...
/obj/machinery/computer/auxiliary_base/ui_data(mob/user)
	...
	if(LAZYLEN(turrets))
		for(var/turret in turrets)
			...
			var/turret_integrity = max((base_turret.atom_integrity - base_turret.integrity_failure * base_turret.max_integrity) / (base_turret.max_integrity - base_turret.integrity_failure * max_integrity) * 100, 0)
			var/turret_status
