...
//					random_species()
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("newdiscipline")
					if((player_experience < 10) || !(pref_species.id == "kindred") || !(clan.name == "Caitiff"))
						return
				...
				if("newvaleren")
					if((player_experience < 10) || !(pref_species.id == "kindred") || !((clan.name == "Salubri") || (clan.name == "Salubri Warrior")))
						return
				...
				if("generation")
					if((clan?.name == "Caitiff") || (player_experience < 20))
						return
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1, roundstart_checks = TRUE, character_setup = FALSE, antagonist = FALSE, is_latejoiner = TRUE, loadout = FALSE) // TFN EDIT: original: remove "loadout = FALSE"
	...
	if(pref_species.name == "Vampire")
		...
		character.clan.current_accessory = clane_accessory
