...
/datum/discipline_power/thaumaturgy/activate(atom/target)
	...
	success_count = SSroll.storyteller_roll(dice = owner.st_get_stat(STAT_WILLPOWER), difficulty = (level + 3), numerical = TRUE, mobs_to_show_output = owner, force_chat_result = TRUE)
	if(success_count < 0)
