...
/obj/effect/ritual/wash(clean_types)
	. = ..()
	qdel(src)

/obj/effect/ritual/examine(mob/user)
	. = ..()
	if(!IS_HERETIC(user))
		return
...
/obj/effect/ritual/attack_paw(mob/living/user, list/modifiers)
	return attack_hand(user, modifiers)

/obj/effect/ritual/can_interact(mob/living/user)
	. = ..()
	if(!.)
		return
	if(!IS_HERETIC(user))
...
/obj/effect/ritual/interact(mob/living/user)
	. = ..()
 ...
 */
/obj/effect/ritual/proc/try_rituals(mob/living/carbon/human/user)
	is_in_use = TRUE

	var/list/rituals = user.get_rituals()
	if(!length(rituals))
	...
	if(!chosen || !istype(rituals[chosen], /datum/ritual_knowledge) || QDELETED(src) || QDELETED(user))
		is_in_use = FALSE
		return
 ...
 * * knowledge_list - a non-assoc list of ritual_knowledge datums.
 *
 * returns TRUE if any rituals passed succeeded, FALSE if they all failed.
 */
/obj/effect/ritual/proc/do_ritual(mob/living/user, datum/ritual_knowledge/ritual)
...
/obj/effect/ritual/big
	icon = 'icons/effects/96x96.dmi'
	icon_state = "transmutation_rune"
	pixel_x = -30
	pixel_y = 18
	pixel_z = -48
	greyscale_config = /datum/greyscale_config/ritual

/obj/effect/ritual/big/Initialize(mapload, path_colour)
	. = ..()
	if (path_colour)
		set_greyscale(colors = list(path_colour))

/obj/effect/temp_visual/drawing_ritual
	duration = 30 SECONDS
	icon = 'icons/effects/96x96.dmi'
	icon_state = "transmutation_rune"
	pixel_x = -30
...
/obj/effect/temp_visual/drawing_ritual/Initialize(mapload, path_colour = COLOR_WHITE)
	. = ..()
	set_greyscale(colors = list(path_colour))
	icon_state = animation_state

/obj/effect/temp_visual/drawing_ritual/fast
	duration = 12 SECONDS
	animation_state = "transmutation_rune_fast"

/obj/effect/temp_visual/drawing_ritual/fail
